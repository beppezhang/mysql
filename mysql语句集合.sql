...
5：多字段合并为一个字段显示：
SELECT
	c.secret_id,c.name,c.gender,c.mobile,c.tel,c.appointment_date,c.intention_vehicle,c.create_time,sp.industry_tag1,cp1.value,cp2.`value`,cr.custom_properties_3,crm.message,CONCAT_WS(
	'  ',
	c.remark,
	ks.custom_properties3
) AS remark
FROM
	customer c
INNER JOIN secret_pool sp on c.secret_id=sp.secret_id
INNER JOIN call_record cr ON cr.secret_id = c.secret_id 
LEFT JOIN keep_secret ks on ks.secret_id=c.secret_id
LEFT JOIN custom_properties cp1 on cr.custom_properties_id1=cp1.id
LEFT JOIN custom_properties cp2 on cr.custom_properties_id2=cp2.id
INNER JOIN caller_record_msg crm on cr.session_id=crm.session_id
